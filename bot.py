...
# ---------- Router: auto-selección de modelo + thinking ---------- #
...
async def apply_routing(session: ChatSession, prompt: str) -> None:
    ...
    async with get_session_lock(session.chat_id):
        if session.auto_route:
            target_model, target_thinking = route_prompt(prompt)  # ahora es síncrona
        else:
